...
options.backuppc_ip = "192.168.1.77"
options.public_key = %Q!<insert public key from backuppc user on server>!
...
def edit_sudoers(user,ip)
  begin
    File.open('/etc/sudoers', 'a') do |f|
      ...
      rule = "#{user['name']} localhost=(root) NOPASSWD: /usr/bin/rsync"
      f.puts rule
...
def create_user(user,os_version)
  if os_version  >= "10.5" and os_version < "10.6" then
